...
현재 프로젝트에서 가장 추천하는 첫 작업은 게시글 AI 분석 연결 재구현이다.

바로 사용할 수 있는 프롬프트:

```text
AGENTS.md와 docs 문서를 먼저 읽어줘.
아직 코드를 수정하지 말고, 현재 프로젝트 상태를 기준으로 게시글 AI 분석 연결을 어떻게 다시 구현하면 좋을지 설계 코치 역할로 설명해줘.

목표는 먼저 React 상세 화면에서 AI 분석 버튼을 누르면
NestJS POST /posts/:id/analyze를 거쳐
FastAPI /analysis/demo까지 요청이 갔다가
분석 결과가 다시 화면에 표시되는 흐름을 완성하는 거야.

조건:
- React가 FastAPI를 직접 호출하지 않는다.
- 인증과 작성자 검사는 NestJS에서 처리한다.
- FastAPI는 demo 분석 결과만 반환해도 된다.
- RAG, OpenAI, MCP, Agent는 이 연결이 된 뒤 다음 작업으로 미룬다.
