...
| WorkoutPost 모델 생성 | Prisma schema에 작성 |
| WorkoutExercise 모델 생성 | Prisma schema에 작성 |
| WorkoutSet 모델 생성 | Prisma schema에 작성 |
...
git switch -c feat/workout-post-basic-api
```

이미 브랜치가 있다고 나오면:

```bash
git switch feat/workout-post-basic-api
```
...
```text
User 1명은 WorkoutPost 여러 개를 작성할 수 있다.
```
...
```bash
npx prisma migrate dev --name init_workout_posts
```
...
```text
src/posts/dto/create-workout-set.dto.ts
src/posts/dto/create-workout-exercise.dto.ts
src/posts/dto/create-workout-post.dto.ts
...
`backend/src/posts/dto/create-workout-set.dto.ts`
...
`backend/src/posts/dto/create-workout-exercise.dto.ts`
...
`backend/src/posts/dto/create-workout-post.dto.ts`
...
```text
demo@workout.local 사용자가 있으면 재사용
없으면 생성
```
...
- [ ] `npx prisma migrate dev --name init_workout_posts`가 성공한다.
...
git push -u origin feat/workout-post-basic-api
```
...
PostsService.create()
        ↓
Prisma workoutPost.create()
        ↓
WorkoutPost + WorkoutExercise + WorkoutSet 저장
        ↓
JSON 응답 반환
```
